...
| 12 Sep | AutoRec (I-AutoRec item-based autoencoder, PyTorch) — notebook Steps 9/10: 24-combo grid (k x weight_decay) tuned on validation, RMSE-best (k=200, wd=1e-4) and NDCG-best (k=500, wd=1e-2) diverge again (same lesson as Step 4's SVD centering choice); AutoRec wins on test RMSE (0.8517 vs SVD's 0.8734, user-mean's 0.9448) but loses on ranking (NDCG@10 0.0948 vs SVD 0.1502, weighted Hybrid 0.1616, Most-Popular 0.1549) because its recommendations concentrate on popular items (1.1% catalogue coverage); cold-item degeneracy confirmed exactly as with SVD (std=0.00e+00 across 732 untrained items per user) | `fa6697e` |
